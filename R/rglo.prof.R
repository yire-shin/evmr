#' @name rglo.prof
#' @aliases rglo.prof
#' @title profile likelihood for rglo
#' @param z An object returned by \code{rglofit}. The object should represent a stationary model.
...
#' \dontrun{
#' z<-rglo.fit(bangkok)
#' rglo.Prof(z,100,xlow=200,xup=500)
#' }
rglo.Prof <- function(z, m, xlow, xup, conf = 0.95, nint = 100) {
  ...
  rglo.plik <- function(a) {
    ...
    z1 <- as.matrix(z$data[, 1],ncol=1)
    zr <- as.matrix(z$data[, r],ncol=1)

    ri <- (r - seq(1, r))
    cr <- (1 + ri)

    mu <- xp - a[1] / a[2] + (a[1] / a[2]) * exp(a[2] * log((1 / p) - 1))

    # Check constraints for y, f, and mu values
    if (any(a[1] <= 0) | any(cr < 0)) return(10^6)

    y <- 1 - a[2] * (z$data - mu) / a[1]
    f <- 1 + (1 - a[2] * (zr - mu) / a[1])^(1 / a[2])
    cond <- (a[1] / a[2]) + mu

    if (any(a[2] > 0)) {
      if (any(cond <= z1, na.rm = TRUE)) return(10^6)
    } else if (any(a[2] < 0)) {
      if (any(cond >= zr, na.rm = TRUE)) return(10^6)
    }

    if (any(y <= 0) || any(f <= 0)) return(10^6)
  ...
  for (i in 1:nint) {
    xp <- x[i]
    opt <- try(suppressWarnings(stats::optim(sol, rglo.plik, control = list(trace = 0))))
  ...
  plot(d$x, d$v, type = "l", xlab = "Return level", xlim = c(xlow, xup), las=1,
       ylab = "Profile Log-likelihood", main = sprintf("Profile Likelihood for CI of %syear Return level in the rGLD (r = %s)", m, z$r))
  ma <- -z$nllh
